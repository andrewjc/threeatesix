...
func INSTR_JZ_SHORT_REL8(core *CpuCore) {
	...
	if core.registers.GetFlag(ZeroFlag) {
		core.registers.IP = uint16(destAddr)
		log.Printf("[%#04x]   |-> jumped", core.GetCurrentlyExecutingInstructionPointer())
	} else {
		core.registers.IP = uint16(uint16(core.GetIP() + 2))
		log.Printf("[%#04x]   |-> no jump", core.GetCurrentlyExecutingInstructionPointer())
	}
}
...
func INSTR_JMP_SHORT_REL8(core *CpuCore) {
	...
	log.Printf("[%#04x] JMP %#04x (SHORT REL8)", core.GetCurrentlyExecutingInstructionPointer(), uint16(destAddr))
	core.registers.IP = uint16(destAddr)
